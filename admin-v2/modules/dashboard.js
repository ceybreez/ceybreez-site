...
const Dashboard = {
  ...
  renderStats(inquiries, bookings) {
    ...
    const todayDepartures = bookedBookings.filter(b => b.dateTo === today).length;

    const pendingPayments = bookings.filter(b => {
      const payment = String(b.paymentStatus || "Pending").toLowerCase();
  ...
  renderRecentActivity(inquiries, bookings) {
    ...
    const inquiryItems = inquiries.map(i => ({
      ...
      guest: i.guestName || i.name || "Guest",
      item: i.itemName || i.serviceType || "CeyBreez Inquiry",
      status: i.status || "New",
    ...
    const bookingItems = bookings.map(b => ({
      ...
      item: b.itemName || b.serviceType || "CeyBreez Booking",
      status: b.status || "Booked",
      date: b.createdAt || b.created_at || b.dateFrom || ""
    }));

    const activity = [...inquiryItems, ...bookingItems]
    ...
    box.innerHTML = activity.map(item => `
      <div class="activity-item">
        <div>
          ...
          <div class="text-soft">
            ${this.escape(item.guest)} · ${this.escape(item.item)}
          </div>
  ...
  renderRevenueChart(bookings) {
    ...
    bookings.forEach(b => {
      ...
      const amount = this.moneyNumber(b.totalAmount || b.quoteTotalAmount);
      months[date] = (months[date] || 0) + amount;
    });

    const rows = Object.entries(months).sort().slice(-6);

    if (!rows.length) {
      box.innerHTML = "No revenue data yet";
      return;
    }
    ...
    box.innerHTML = `
      <div class="chart-bars">
        ${rows.map(([month, amount]) => `
          <div class="chart-row">
            <span>${this.escape(month)}</span>
            <div class="bar-wrap">
              <div class="bar" style="width:${(amount / max) * 100}%"></div>
            </div>
            <strong>OMR ${amount.toFixed(3)}</strong>
          </div>
        `).join("")}
      </div>
    `;
  ...
  calculateOccupancy(bookings) {
    ...
    const uniqueItems = new Set(
      bookings.map(b => String(b.itemName || "").trim()).filter(Boolean)
    ).size;
  ...
  badgeClass(status) {
    ...
    if (s.includes("closed")) return "badge-cancelled";
