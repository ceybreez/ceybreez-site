...
body.cb-builder-tablet [data-cb-responsive="flex"],body.cb-builder-tablet [data-cb-responsive="row"]{display:flex!important;flex-wrap:nowrap!important;align-items:stretch!important;gap:18px!important;width:100%!important;max-width:100%!important}
body.cb-builder-tablet [data-cb-responsive-item="1"]{min-width:0!important;max-width:100%!important;transform:none!important;left:auto!important;right:auto!important;top:auto!important;bottom:auto!important}
body.cb-builder-tablet [data-cb-responsive="flex"]>[data-cb-responsive-item="1"],body.cb-builder-tablet [data-cb-responsive="row"]>[data-cb-responsive-item="1"]{position:relative!important;flex:1 1 0!important;width:auto!important}
body.cb-builder-tablet img[data-cb-responsive-item="1"],body.cb-builder-tablet [data-cb-responsive-item="1"]>img{width:100%!important;height:100%!important;min-height:180px!important;object-fit:cover!important}
...
body.cb-builder-mobile [data-cb-responsive-item="1"]{
  ...
  margin-left:auto!important;margin-right:auto!important;left:auto!important;right:auto!important;top:auto!important;bottom:auto!important;
  position:relative!important;transform:none!important;
}
body.cb-builder-mobile [data-cb-responsive-item="1"] img,
body.cb-builder-mobile img[data-cb-responsive-item="1"]{display:block!important;width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;object-fit:cover!important;visibility:visible!important;opacity:1!important}
...
body.cb-builder-mobile nav{width:100%!important;max-width:100%!important;min-width:0!important;overflow:visible!important;flex-wrap:wrap!important;white-space:normal!important;justify-content:center!important;gap:6px!important;padding-left:8px!important;padding-right:8px!important}
body.cb-builder-mobile nav a{white-space:normal!important;max-width:100%!important;overflow-wrap:anywhere!important}
...
function updateContextPanels(){
 ...
 $('mediaGroup').classList.toggle('context-hidden',!['IMG','VIDEO'].includes(tag));
}
...
function selectedPageSection(){if(!selected)return null;const section=selected.closest('section');if(!section)return null;const main=section.closest('main');return main&&section.parentElement===main?section:null}
function normalizeSectionFlow(section){if(!section)return;['transform','top','right','bottom','left'].forEach(p=>section.style.removeProperty(p));section.style.setProperty('position','relative','important');section.style.setProperty('display','block','important');section.style.setProperty('width','100%','important');section.style.setProperty('max-width','100%','important');section.style.setProperty('visibility','visible','important');section.style.setProperty('opacity','1','important')}
function moveSelected(direction){if(!selected||!selected.parentElement)return;const section=selectedPageSection(),node=section||selected,parent=node.parentElement;const siblings=[...parent.children].filter(x=>section?x.tagName==='SECTION':true),index=siblings.indexOf(node),target=siblings[index+direction];if(!target){setStatus(direction<0?'This is already the first section / element.':'This is already the last section / element.');return}history.push(snapshot());if(section)normalizeSectionFlow(section);if(direction<0)parent.insertBefore(node,target);else parent.insertBefore(target,node);recordChange(parent);recordChange(node);buildSections();buildLayers();positionSelectionOverlay();setStatus(section?(direction<0?'Section moved up.':'Section moved down.'):(direction<0?'Element moved up.':'Element moved down.'))}
function changeStacking(front){if(!selected||!selected.parentElement)return;const values=[...selected.parentElement.children].map(el=>parseInt(frame.contentWindow.getComputedStyle(el).zIndex,10)).filter(Number.isFinite),edge=values.length?(front?Math.max(...values):Math.min(...values)):0;mutate(el=>{if(frame.contentWindow.getComputedStyle(el).position==='static')setUserStyle(el,'position','relative');setUserStyle(el,'z-index',front?edge+1:edge-1)});setStatus(front?'Element brought to front.':'Element sent behind.')}
...
function startMove(e){if(!selected)return;e.preventDefault();e.stopPropagation();const section=selectedPageSection(),d=frame.contentDocument;if(section){selected=section;selectElement(section);history.push(snapshot());normalizeSectionFlow(section);resizeState={mode:'section-reorder',parent:section.parentElement,moved:false};d.body.classList.add('cb-reordering-section');setStatus('Drag the section up or down and release it in the required position.')}else{const cs=frame.contentWindow.getComputedStyle(selected),matrix=new DOMMatrixReadOnly(cs.transform==='none'?'matrix(1,0,0,1,0,0)':cs.transform);resizeState={mode:'move',startX:e.clientX,startY:e.clientY,tx:matrix.m41,ty:matrix.m42}}d.addEventListener('mousemove',doMove,true);d.addEventListener('mouseup',stopMove,true)}
function doMove(e){if(!resizeState||!selected)return;e.preventDefault();if(resizeState.mode==='section-reorder'){const hit=frame.contentDocument.elementFromPoint(e.clientX,e.clientY),target=hit?.closest('section');if(!target||target===selected||target.parentElement!==resizeState.parent)return;const r=target.getBoundingClientRect();resizeState.parent.insertBefore(selected,e.clientY<r.top+r.height/2?target:target.nextElementSibling);resizeState.moved=true;positionSelectionOverlay();return}if(resizeState.mode==='move'){const x=resizeState.tx+(e.clientX-resizeState.startX),y=resizeState.ty+(e.clientY-resizeState.startY);setUserStyle(selected,'transform',`translate(${Math.round(x)}px, ${Math.round(y)}px)`);positionSelectionOverlay()}}
function stopMove(){const d=frame.contentDocument;d.removeEventListener('mousemove',doMove,true);d.removeEventListener('mouseup',stopMove,true);d.body.classList.remove('cb-reordering-section');if(resizeState?.mode==='section-reorder'){if(resizeState.moved){recordChange(resizeState.parent);recordChange(selected);buildSections();buildLayers();setStatus('Section placed in the new page position.')}else{recordChange(selected);setStatus('Section kept in normal page flow.')}}else if(selected){recordChange(selected);setStatus('Element moved.')}resizeState=null;positionSelectionOverlay()}
...
function recordChange(el){if(!el)return;const s=selectorFor(el);if(!s)return;const draft=getDraft(),record={selector:s,html:el.innerHTML,attrs:{},style:el.getAttribute('style')||'',hover:draft[s]?.hover||null};['href','src','class','id','controls','autoplay','loop','muted','playsinline'].forEach(a=>{if(el.hasAttribute(a))record.attrs[a]=el.getAttribute(a)});draft[s]=cleanRecord(record);localStorage.setItem(PREFIX+pageKey,JSON.stringify(draft));const removed=getRemoved();removed.delete(s);setRemoved(removed);updateInspector();buildLayers()}
...
bind('minWidthValue','change',(e,v)=>setUserStyle(e,'min-width',v));
bind('maxHeightValue','change',(e,v)=>setUserStyle(e,'max-height',v));
bind('positionValue','change',(e,v)=>setUserStyle(e,'position',v));
bind('overflowValue','change',(e,v)=>setUserStyle(e,'overflow',v));
...
bind('objectFit','change',(e,v)=>setUserStyle(e,'object-fit',v));
bind('objectPosition','change',(e,v)=>setUserStyle(e,'object-position',v));
bind('transitionValue','change',(e,v)=>setUserStyle(e,'transition',v));
...
$('duplicateBtn').onclick=duplicateSelected;$('deleteBtn').onclick=deleteSelected;$('duplicateQuickBtn').onclick=duplicateSelected;$('deleteQuickBtn').onclick=deleteSelected;$('moveUpBtn').onclick=()=>moveSelected(-1);$('moveDownBtn').onclick=()=>moveSelected(1);$('bringFrontBtn').onclick=()=>changeStacking(true);$('sendBackBtn').onclick=()=>changeStacking(false);
function addBlock(type){const d=frame.contentDocument,main=d.querySelector('main')||d.body,anchor=selected?.closest?.('section'),canContain=selected&&!['IMG','VIDEO','INPUT','TEXTAREA','HR'].includes(selected.tagName);let parent=canContain?selected:main;let el;const make=h=>{const t=d.createElement('template');t.innerHTML=h.trim();return t.content.firstElementChild};const map={section:'<section style="display:block;position:relative;width:100%;max-width:100%;min-height:120px;padding:70px 20px;visibility:visible;opacity:1"><h2>New Section</h2><p>Add your content here.</p></section>',container:'<div style="max-width:1200px;margin:auto;padding:20px">Container</div>',columns:'<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px"><div>Column 1</div><div>Column 2</div></div>',heading:'<h2>New Heading</h2>',text:'<p>New text content</p>',button:'<a href="#" style="display:inline-block;padding:12px 22px;border-radius:6px;background:#087f75;color:#fff;text-decoration:none">Button</a>',image:'<img src="images/cover.jpg" alt="New image" style="max-width:100%;height:auto">',video:'<video controls playsinline style="display:block;max-width:100%;height:auto"></video>',card:'<div style="padding:24px;border-radius:12px;background:#fff;box-shadow:0 10px 30px rgba(0,0,0,.12)"><h3>Card title</h3><p>Card content</p></div>',spacer:'<div style="height:60px"></div>',divider:'<hr style="border:0;border-top:1px solid #d8e2e8">'};el=make(map[type]);el.id=`cb-custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;if(type==='section'){parent=main;if(anchor&&anchor.parentElement===main)anchor.after(el);else main.appendChild(el)}else parent.appendChild(el);stableSelectors.set(el,'#'+CSS.escape(el.id));recordChange(parent);selectElement(el);recordChange(el);buildSections();el.scrollIntoView({behavior:'smooth',block:'center'});setStatus(type==='section'?'New section added after the selected page section. Use ↑/↓ or the ✥ handle to reorder it.':`${type} added.`)}
...
function startUpload(target){uploadTarget=target;$('fileInput').accept=target.includes('video')?'video/*':'image/*';$('fileInput').click()}$('uploadBtn').onclick=()=>startUpload('image');$('mediaUploadBtn').onclick=()=>startUpload('image');$('videoUploadBtn').onclick=()=>{addBlock('video');startUpload('video')};$('bgUploadBtn').onclick=()=>startUpload('background');$('bgVideoUploadBtn').onclick=()=>startUpload('background-video');$('fileInput').onchange=async e=>{const file=e.target.files[0];if(!file)return;const token=localStorage.getItem('CEYBREEZ_ADMIN_TOKEN');if(!token){setStatus('Admin token not found. Log in to Admin first.',true);return}const fd=new FormData();fd.append('file',file);fd.append('folder','page-builder');setStatus(`Uploading ${file.type.startsWith('video/')?'video':'image'}…`);try{const r=await fetch(API_BASE+'/api/admin/upload-image',{method:'POST',headers:{Authorization:`Bearer ${token}`},body:fd});const data=await r.json();if(!r.ok||!data.url)throw Error(data.error||'Upload failed');if(selected){if(uploadTarget==='background')mutate(el=>setUserStyle(el,'background-image',`url("${data.url}")`));else if(uploadTarget==='background-video')applyBackgroundVideo(selected,data.url);else if(selected.tagName==='IMG'||selected.tagName==='VIDEO')mutate(el=>el.setAttribute('src',data.url));else{const media=frame.contentDocument.createElement(file.type.startsWith('video/')?'video':'img');media.src=data.url;media.style.maxWidth='100%';if(media.tagName==='VIDEO'){media.controls=true;media.playsInline=true}selected.appendChild(media);recordChange(selected)}}setStatus('Media uploaded and applied.')}catch(err){setStatus(err.message,true)}e.target.value=''};

function applyBackgroundVideo(el,url){mutate(target=>{target.style.position=target.style.position||'relative';let v=target.querySelector(':scope > .cb-user-bg-video');if(!v){v=frame.contentDocument.createElement('video');v.className='cb-user-bg-video';v.autoplay=true;v.muted=true;v.loop=true;v.playsInline=true;v.style.cssText='position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;pointer-events:none';target.prepend(v);[...target.children].filter(x=>x!==v).forEach(x=>{if(!x.style.position)x.style.position='relative';x.style.zIndex=x.style.zIndex||'1'})}v.src=url});$('backgroundVideo').value=url}
function setHover(){if(!selected)return;recordChange(selected);const s=selectorFor(selected),draft=getDraft();draft[s].hover={background:$('hoverBackground').value,color:$('hoverColor').value,transform:$('hoverTransform').value,transition:$('transitionValue').value||'all .25s ease'};localStorage.setItem(PREFIX+pageKey,JSON.stringify(draft));let st=frame.contentDocument.getElementById('cb-hover-preview');if(!st){st=frame.contentDocument.createElement('style');st.id='cb-hover-preview';frame.contentDocument.head.appendChild(st)}st.textContent=Object.values(draft).filter(x=>x.hover).map(x=>`${x.selector}{transition:${x.hover.transition}}${x.selector}:hover{background:${x.hover.background}!important;color:${x.hover.color}!important;transform:${x.hover.transform||'none'}!important}`).join('\n')}
$('hoverBackground').oninput=setHover;$('hoverColor').oninput=setHover;$('hoverTransform').onchange=setHover;
$('backgroundMode').onchange=e=>mutate(el=>{const mode=e.target.value;if(mode==='none'){el.style.removeProperty('background');el.style.removeProperty('background-image')}if(mode==='color')setUserStyle(el,'background',$('bgValue').value);if(mode==='gradient')setUserStyle(el,'background',`linear-gradient(135deg,${$('gradientStart').value},${$('gradientEnd').value})`);if(mode==='image'&&$('backgroundImage').value)setUserStyle(el,'background-image',`url("${$('backgroundImage').value}")`);if(mode==='video'&&$('backgroundVideo').value)applyBackgroundVideo(el,$('backgroundVideo').value)});
function applyGradient(){if(selected)mutate(el=>setUserStyle(el,'background',`linear-gradient(135deg,${$('gradientStart').value},${$('gradientEnd').value})`))}$('gradientStart').oninput=applyGradient;$('gradientEnd').oninput=applyGradient;$('backgroundVideo').onchange=e=>{if(selected&&e.target.value)applyBackgroundVideo(selected,e.target.value)};$('clearBgVideoBtn').onclick=()=>{if(!selected)return;mutate(el=>el.querySelector(':scope > .cb-user-bg-video')?.remove());$('backgroundVideo').value=''};
