...
$('saveBtn').onclick=()=>{
 try{
  ...
  if(!d||!d.body)throw new Error('Page preview is not ready');
  if(selected)recordChange(selected);
  [...d.querySelectorAll('[data-cb-edit-id],[data-cb-user-edited="1"]')].forEach(el=>recordChange(el));
  const draft=getDraft();
  const key=PREFIX+pageKey;
  const payload=JSON.stringify(draft);
  localStorage.setItem(key,payload);
  const check=localStorage.getItem(key);
  if(check!==payload)throw new Error('Browser storage verification failed');
  const count=Object.keys(JSON.parse(check||'{}')).length;
  setStatus(`SAVED ✓ ${pageConfig(pageKey).label} — ${count} edited item(s). Reloading this page will keep the draft.`);
  $('saveBtn').textContent='Saved ✓';
  setTimeout(()=>{$('saveBtn').textContent='Save draft'},1600);
 }catch(err){
  setStatus('Draft save failed: '+err.message+'. Check that browser storage is allowed.',true);
 }
