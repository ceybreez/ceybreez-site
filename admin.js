...
let inquiryViewMode = 'property';
let bookingViewMode = 'property';
...
function showTab(tab) {
  if (tab === "propertyInquiries") {
    inquiryViewMode = "property";
    tab = "inquiriesTab";
  }

  if (tab === "tourInquiries") {
    inquiryViewMode = "tour";
    tab = "inquiriesTab";
  }

  if (tab === "propertyBookings") {
    bookingViewMode = "property";
    tab = "bookingsTab";
  }

  if (tab === "tourBookings") {
    bookingViewMode = "tour";
    tab = "bookingsTab";
  }
...
function isTourRecord(item){
  const text = `${item?.serviceType || ""} ${item?.itemName || ""} ${item?.type || ""}`.toLowerCase();
  return text.includes("tour");
}

function isPropertyRecord(item){
  return !isTourRecord(item);
}

function viewModeMatch(item, mode){
  if(mode === "tour") return isTourRecord(item);
  if(mode === "property") return isPropertyRecord(item);
  return true;
}

function updateInquiryViewHeadings(){
  const isTour = inquiryViewMode === "tour";
  const title = isTour ? "Tour Inquiry" : "Property Inquiry";
  const listTitle = isTour ? "Latest Tour Inquiries" : "Latest Property Inquiries";
  const subTitle = isTour
    ? "Manage tour inquiries, quotes, guest confirmation and admin confirmation."
    : "Manage villa, homestay and apartment inquiries, quotes, guest confirmation and admin confirmation.";

  const pageTitle = document.getElementById("inquiryPageTitle");
  const listHead = document.getElementById("inquiryListTitle");
  const listSub = document.getElementById("inquiryListSubTitle");

  if(pageTitle) pageTitle.textContent = title;
  if(listHead) listHead.textContent = listTitle;
  if(listSub) listSub.textContent = subTitle;
}

function updateBookingViewHeadings(){
  const isTour = bookingViewMode === "tour";
  const title = isTour ? "Tour Booking Management" : "Property Booking Management";
  const listTitle = isTour ? "Tour Bookings List" : "Property Bookings List";
  const subTitle = isTour
    ? "Confirmed tour bookings, cancelled tour records and tour booking calendar."
    : "Confirmed villa, homestay and apartment bookings with availability calendar.";

  const pageTitle = document.getElementById("bookingPageTitle");
  const listHead = document.getElementById("bookingListTitle");
  const listSub = document.getElementById("bookingListSubTitle");
  ...
  if(typeFilter){
    typeFilter.value = isTour ? "tour" : "all";
  }
}
...
/* =========================
   CMS TABLE HELPERS
========================= */
function toggleCmsForm(id){
  const box = document.getElementById(id);
  if(!box) return;

  if(box.classList.contains("hidden")) showCmsForm(id);
  else box.classList.add("hidden");
}

function prettyFieldLabel(el){
  const fromPlaceholder = el.getAttribute("placeholder");
  if(fromPlaceholder) return fromPlaceholder.replace(" / ", " / ");

  const id = el.id || "Field";
  return id
    .replace(/^(dest|prop|service|section)/, "")
    .replace(/([A-Z])/g, " $1")
    .replace(/^./, c => c.toUpperCase())
    .trim();
}

function ensureCmsFieldLabels(box){
  if(!box) return;

  box.querySelectorAll("input, textarea, select").forEach(el => {
    const type = String(el.type || "").toLowerCase();
    if(type === "hidden" || type === "file" || type === "checkbox" || type === "color") return;
    if(el.closest("label")) return;

    let previous = el.previousElementSibling;
    while(previous && previous.classList?.contains("upload-box")) previous = previous.previousElementSibling;
    if(previous && previous.tagName && previous.tagName.toLowerCase() === "label") return;

    const label = document.createElement("label");
    label.className = "auto-field-label";
    label.textContent = prettyFieldLabel(el);
...
function showCmsForm(id){
  ...
  box.classList.add("cms-modal-form");
  ensureCmsFieldLabels(box);

  if(!box.querySelector(".cms-modal-header")){
    const header = document.createElement("div");
    header.className = "cms-modal-header";
    header.innerHTML = `
      <h3>${escapeHtml(titles[id] || "Edit Details")}</h3>
      <button type="button" class="cms-modal-close">× Close</button>
    `;
    header.querySelector("button").onclick = () => box.classList.add("hidden");
    box.prepend(header);
  }

  document.body.classList.add("cms-modal-active");
}
...
function openAddDestinationForm(){ resetDestinationForm(); showCmsForm("destinationFormBox"); }
...
function openAddPropertyForm(type="villa"){ resetPropertyForm(); document.getElementById("propType").value=type; showCmsForm("propertyFormBox"); }
...
async function savePageSection(e) {
  ...
  const settings = {
    ...
    cards: collectCards(),
    seoTitle: document.getElementById("sectionSeoTitle")?.value || "",
    seoDescription: document.getElementById("sectionSeoDescription")?.value || ""
  };
...
async function editPageSection(id) {
  ...
  if (document.getElementById("sectionSeoTitle")) document.getElementById("sectionSeoTitle").value = settings.seoTitle || "";
  if (document.getElementById("sectionSeoDescription")) document.getElementById("sectionSeoDescription").value = settings.seoDescription || "";
  if (document.getElementById("sectionButtonText")) document.getElementById("sectionButtonText").value = item.buttonText || settings.buttonText || "";
  if (document.getElementById("sectionButtonUrl")) document.getElementById("sectionButtonUrl").value = item.buttonUrl || settings.buttonUrl || "";
  loadCards(settings.cards || []);

  showCmsForm("sectionBuilderBox");
}
...
function openAddServiceForm(){ resetServiceForm(); showCmsForm("serviceFormBox"); }
...
async function loadInquiries() {
  try {
    ...
    const visibleInquiries = allInquiries.filter(x => viewModeMatch(x, inquiryViewMode));
...
function applyInquiryFilters() {
  ...
  const filtered = allInquiries.filter(item => {
    ...
    return viewModeMatch(item, inquiryViewMode) &&
    ...
    card.innerHTML = `
      ...
      <select class="inquiry-status">
        ...
        <option value="Booked">Booked</option>
          <option value="Guest Confirmed">Guest Confirmed</option>
...
function renderInquiryTable(data) {
  ...
  if (!data.length) {
    tbody.innerHTML = `
      <tr>
        <td colspan="10" class="empty-row">No inquiries found</td>
      </tr>
    `;
    return;
  }
  ...
  tbody.innerHTML = data.map(item => {
    const statusClass = normalizeStatus(item.status);
    const displayStatus = item.status || "New";
    const guestConfirmed = Number(item.guestConfirmed || 0) === 1 || normalizeStatus(item.status) === "guest confirmed";
    const adminConfirmed = Number(item.adminConfirmed || 0) === 1 || normalizeStatus(item.status) === "admin confirmed" || normalizeStatus(item.status) === "booked";
    const quoteTotal = item.quoteTotalAmount || item.totalAmount || "";
    const quoteCurrency = item.quoteCurrency || item.currency || "";
    ...
    return `
      <tr onclick="openInquiryModal('${escapeJs(item.id)}')" class="clickable-row">
        ...
        <td>
          ...
          <small>${escapeHtml(item.guestEmail || "")}</small>
        </td>
        <td>${yesNoBadge(guestConfirmed, "Guest OK", "Pending")}</td>
        <td>${yesNoBadge(adminConfirmed, "Admin OK", "Pending")}</td>
        <td>
          <strong>${escapeHtml(item.quoteStatus || "-")}</strong><br>
          <small>${escapeHtml(moneyText(quoteCurrency, quoteTotal))}</small>
        </td>
        <td>
        ...
        <td onclick="event.stopPropagation();">
          <button class="mini-btn" onclick="openInquiryModal('${escapeJs(item.id)}')">Manage</button>
        </td>
      </tr>
    `;
  }).join("");
...
function renderAvailabilityMatrix(){
  ...
  const properties = selectedType === "tour"
    ? (allDestinations || []).map(d => ({ ...d, type: "tour", location: d.province || d.area || "" }))
...
async function loadBookings() {
  ...
  try {
    ...
    allBookings = data || [];
    renderBookingStats(allBookings.filter(x => viewModeMatch(x, bookingViewMode)));
    applyBookingFilters();
    renderReports();
  } catch (err) {
    if (tableBody) {
...
function applyBookingFilters() {
  ...
  const filtered = allBookings.filter(item => {
    ...
    return viewModeMatch(item, bookingViewMode) && statusMatch && typeMatch && (!search || searchText.includes(search));
  });
...
function openBookingDetails(id){
  ...
  box.innerHTML = `
    <div class="booking-detail-card">
      ...
      <p><b>Currency:</b> ${escapeHtml(booking.currency || "-")}</p>
      <p><b>Day Rate:</b> ${escapeHtml(booking.dayRate || booking.adultRate || "-")}</p>
      <p><b>Discount:</b> ${escapeHtml(booking.discountPercent || "0")}% (${escapeHtml(booking.discountAmount || "0")})</p>
      <p><b>Total Amount:</b> ${escapeHtml(booking.totalAmount || booking.finalAmount || "-")}</p>
      <p><b>Advance:</b> ${escapeHtml(booking.advanceAmount || "-")}</p>
      <p><b>Balance:</b> ${escapeHtml(booking.balanceAmount || "-")}</p>
      <p><b>Guest Confirm:</b> ${Number(booking.guestConfirmed || 0) === 1 ? "Yes" : "No"}</p>
      <p><b>Admin Confirm:</b> ${Number(booking.adminConfirmed || 0) === 1 ? "Yes" : "No"}</p>
...
function openInquiryModal(id){
  ...
  <button onclick="sendQuoteToGuest()">Send Quote / Invoice</button>
  <button onclick="confirmBooking(document.getElementById('sendStatusEmail')?.checked ?? true)">Admin Confirm Booking</button>
  <button class="delete-btn" onclick="deleteCurrentInquiry()">Delete Inquiry</button>
    ...
    ${renderBookingQuotePanel(inquiry)}
  ...
  [
    "bookingConfirmDayRate",
    "bookingConfirmAdultRate",
    "bookingConfirmChildRate",
    "bookingConfirmGuests",
    "bookingConfirmDiscountPercent",
    "bookingConfirmAdvanceAmount"
  ].forEach(id => document.getElementById(id)?.addEventListener("input", updateBookingConfirmTotals));
...
function isTourInquiry(inquiry){
  const text = `${inquiry?.serviceType || ""} ${inquiry?.itemName || ""}`.toLowerCase();
  return text.includes("tour");
}

function yesNoBadge(value, yesText = "Confirmed", noText = "Pending"){
  return value
    ? `<span class="status-badge status-booked">${yesText}</span>`
    : `<span class="status-badge status-quoted">${noText}</span>`;
}
...
function renderBookingQuotePanel(inquiry){
  const isTour = isTourInquiry(inquiry);
  const validUntil = new Date();
  validUntil.setDate(validUntil.getDate() + 3);
  const validValue = validUntil.toISOString().slice(0,10);

  if(isTour){
    return `
      <div class="modal-section booking-confirm-panel">
        <h4>Tour Quote / Booking Confirmation</h4>

        <div class="booking-confirm-grid booking-confirm-grid-tour">
          <label>Tour Date
            <input id="bookingConfirmTourDate" type="date" value="${escapeHtml(inquiry.dateFrom || "")}">
          </label>

          <label>Pickup Time
            <input id="bookingConfirmPickupTime" type="time" value="08:00">
          </label>

          <label>Pickup Location
            <input id="bookingConfirmPickupLocation" placeholder="Hotel / location">
          </label>
          ...
          <label>Adult Rate
            <input id="bookingConfirmAdultRate" type="number" min="0" step="0.01">
          </label>

          <label>Child Rate
            <input id="bookingConfirmChildRate" type="number" min="0" step="0.01">
          </label>

          <label>Guests
            <input id="bookingConfirmGuests" type="number" min="1" value="${escapeHtml(inquiry.guests || "1")}">
          </label>

          <label>Discount %
            <input id="bookingConfirmDiscountPercent" type="number" min="0" max="100" step="0.01" value="0">
          </label>

          <label>Discount Amount
            <input id="bookingConfirmDiscountAmount" readonly>
          </label>

          <label>Total Amount
            <input id="bookingConfirmTotalAmount" readonly>
          </label>

          <label>Advance Amount
            <input id="bookingConfirmAdvanceAmount" type="number" min="0" step="0.01">
          </label>

          <label>Balance Amount
            <input id="bookingConfirmBalanceAmount" readonly>
          </label>

          <label>Quote Valid Until
            <input id="bookingConfirmValidUntil" type="date" value="${validValue}">
          </label>

          <label>Payment Status
            <select id="bookingConfirmPaymentStatus">
        ...
        <label>Admin Message to Guest
          <textarea id="bookingConfirmAdminMessage" placeholder="Pickup details, inclusions, exclusions, cancellation policy..."></textarea>
        </label>

        <p class="confirm-help">Tour quote total calculates from adult rate × guests minus discount.</p>
      </div>
    `;
  }
  ...
  return `
    <div class="modal-section booking-confirm-panel">
      <h4>Property Quote / Booking Confirmation</h4>

      <div class="booking-confirm-grid">
        <label>Check-in Time
          <input id="bookingConfirmCheckInTime" type="time" value="14:00">
        </label>

        <label>Check-out Time
          <input id="bookingConfirmCheckOutTime" type="time" value="11:00">
        </label>
        ...
        <label>Day / Night Rate
          <input id="bookingConfirmDayRate" type="number" min="0" step="0.01" placeholder="Example: 15000">
        </label>

        <label>Total Nights
          <input id="bookingConfirmNights" readonly>
        </label>

        <label>Gross Amount
          <input id="bookingConfirmGrossAmount" readonly>
        </label>
      ...
      <label>Admin Message to Guest
        <textarea id="bookingConfirmAdminMessage" placeholder="Example: Please arrive after 2.00 PM. Our team will contact you before check-in."></textarea>
      </label>

      <p class="confirm-help">Total and balance calculate automatically from nights, rate, discount, and advance amount.</p>
    </div>
  `;
}

async function sendQuoteToGuest(){
  if(!currentInquiry) return;
  if(!currentInquiry.guestEmail){
  ...
  const res = await fetch(`${API_BASE}/api/admin/inquiries/${currentInquiry.id}/status`, {
    method: "PUT",
    headers: authHeaders(),
    body: JSON.stringify({
      status: "Quoted",
  ...
  alert("Quote / invoice sent to guest");
  await loadInquiries();
  await loadBookings();
  closeInquiryModal();
}
...
function updateBookingConfirmTotals(){
  ...
  const isTour = isTourInquiry(currentInquiry);
  const currency = document.getElementById("bookingConfirmCurrency")?.value || "LKR";
  const discountPercent = Number(document.getElementById("bookingConfirmDiscountPercent")?.value || 0);
  const advance = Number(document.getElementById("bookingConfirmAdvanceAmount")?.value || 0);

  let gross = 0;
  let units = 0;

  if(isTour){
    const guests = Number(document.getElementById("bookingConfirmGuests")?.value || currentInquiry.guests || 1);
    const adultRate = Number(document.getElementById("bookingConfirmAdultRate")?.value || 0);
    units = guests;
    gross = guests * adultRate;
  } else {
    ...
    const grossBox = document.getElementById("bookingConfirmGrossAmount");
    if(nightsBox) nightsBox.value = nights;
    if(grossBox) grossBox.value = gross ? gross.toFixed(2) : "";
  }

  const discountAmount = gross * (discountPercent / 100);
  const total = Math.max(gross - discountAmount, 0);
  const balance = Math.max(total - advance, 0);

  const discountBox = document.getElementById("bookingConfirmDiscountAmount");
  const totalBox = document.getElementById("bookingConfirmTotalAmount");
  const balanceBox = document.getElementById("bookingConfirmBalanceAmount");

  if(discountBox) discountBox.value = discountAmount ? discountAmount.toFixed(2) : "";
  if(totalBox) totalBox.value = total ? total.toFixed(2) : "";
  if(balanceBox) balanceBox.value = total ? balance.toFixed(2) : "";
}
...
function getBookingConfirmDetails(){
  ...
  const discountPercent = document.getElementById("bookingConfirmDiscountPercent")?.value || "0";
  const discountAmount = document.getElementById("bookingConfirmDiscountAmount")?.value || "";
  const advanceAmount = document.getElementById("bookingConfirmAdvanceAmount")?.value || "";
  const balanceAmount = document.getElementById("bookingConfirmBalanceAmount")?.value || "";
  const quoteValidUntil = document.getElementById("bookingConfirmValidUntil")?.value || "";
  const paymentStatus = document.getElementById("bookingConfirmPaymentStatus")?.value || "Pending";
  const adminMessage = document.getElementById("bookingConfirmAdminMessage")?.value || "";

  if(isTour){
    const tourDate = document.getElementById("bookingConfirmTourDate")?.value || currentInquiry.dateFrom || "";
    const guests = document.getElementById("bookingConfirmGuests")?.value || currentInquiry.guests || "";
    const tourEndDate = tourDate ? toDateInputValue(new Date(new Date(tourDate + "T00:00:00").getTime() + 86400000)) : "";
    ...
    return {
      ...
      totalDays: "1",
      guests,
      checkInTime: document.getElementById("bookingConfirmPickupTime")?.value || "",
      checkOutTime: "",
      pickupTime: document.getElementById("bookingConfirmPickupTime")?.value || "",
      pickupLocation: document.getElementById("bookingConfirmPickupLocation")?.value || "",
      adultRate: document.getElementById("bookingConfirmAdultRate")?.value || "",
      childRate: document.getElementById("bookingConfirmChildRate")?.value || "",
      dayRate: document.getElementById("bookingConfirmAdultRate")?.value || "",
      currency,
      discountPercent,
      discountAmount,
      totalAmount,
  ...
  return {
    ...
    checkInTime: document.getElementById("bookingConfirmCheckInTime")?.value || "14:00",
    checkOutTime: document.getElementById("bookingConfirmCheckOutTime")?.value || "11:00",
    dayRate: document.getElementById("bookingConfirmDayRate")?.value || "",
    totalDays: document.getElementById("bookingConfirmNights")?.value || "",
    currency,
    discountPercent,
    discountAmount,
    totalAmount,
    ...
    const res = await fetch(
      `${API_BASE}/api/admin/bookings`,
      {
        ...
        body: JSON.stringify({
          ...
          dateFrom: getBookingConfirmDetails().dateFrom || currentInquiry.dateFrom,
          dateTo: getBookingConfirmDetails().dateTo || currentInquiry.dateTo,
          guests: getBookingConfirmDetails().guests || currentInquiry.guests,
          ...getBookingConfirmDetails(),
          adminConfirmed: 1,
          sendEmail
        })
...
function renderManualPropertyDropdown(properties){
  ...
  const isTour = selectedType.includes("tour");
  const source = isTour ? (allDestinations || []) : (properties || []);
  ...
  const label = isTour ? "Select Tour Location" : "Select Property";
  const optionHtml = `
    <option value="">${label}</option>
    ${filtered.map(p => `
      <option value="${escapeHtml(p.name)}">
        ${escapeHtml(p.name)} ${isTour ? `(${escapeHtml(p.province || p.area || "tour")})` : `(${escapeHtml(p.type || "property")})`}
      </option>
    `).join("")}
  `;

  if(box.tagName.toLowerCase() === "select"){
    box.innerHTML = optionHtml;
    return;
  }
...
function filterManualProperties() {
  ...
  const isTour = typeText.includes("tour");
  let source = isTour ? (allDestinations || []) : (window.allProperties || []);
  let wantedType = "";
  ...
  const filtered = wantedType
    ? source.filter(p => String(p.type || "").toLowerCase() === wantedType)
    : source;

  select.innerHTML = `
    <option value="">${isTour ? "Select Tour Location" : "Select Property"}</option>
    ${filtered.map(p => `
      <option value="${escapeHtml(p.name)}">
        ${escapeHtml(p.name)} ${isTour ? `(${escapeHtml(p.province || p.area || "tour")})` : `(${escapeHtml(p.type || "property")})`}
      </option>
