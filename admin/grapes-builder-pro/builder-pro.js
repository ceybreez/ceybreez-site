(() => {
  ...
  async function fetchOriginal(page){
    ...
    for(const url of candidates){
      try{ const r=await fetch(url,{cache:'no-store'}); if(r.ok){ const html=await r.text(); return extractBodyAndStyles(html,page); } }catch(_){ }
    }
    return fallbackPage(page);
  }

  function extractBodyAndStyles(html,page){
    const doc=new DOMParser().parseFromString(html,'text/html');
    doc.querySelectorAll('script').forEach(s=>s.remove());
    if(page.id==='welcome'){
      const welcome=doc.querySelector('.welcome-screen,#welcomeScreen,[data-welcome],.welcome-overlay');
      if(welcome) return {html:welcome.outerHTML,css:collectCss(doc)};
      return fallbackPage(page);
    }
    return {html:doc.body.innerHTML,css:collectCss(doc)};
  }
  function collectCss(doc){
    const inline=[...doc.querySelectorAll('style')].map(x=>x.textContent).join('\n');
    const links=[...doc.querySelectorAll('link[rel="stylesheet"]')].map(x=>`@import url("${new URL(x.getAttribute('href'),location.href).href}");`).join('\n');
    return `${links}\n${inline}`;
  }
