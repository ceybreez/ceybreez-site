...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".inquiry-search").forEach(input => {
...
/* =========================
   V3 HOME GALLERY CMS
========================= */
function previewHomeGallery(){
  const box=document.getElementById('home_gallery');
  const preview=document.getElementById('homeGalleryPreview');
  if(!box||!preview)return;
  const urls=String(box.value||'').split(/\n|,/).map(x=>x.trim()).filter(Boolean);
  preview.innerHTML=urls.slice(0,18).map(url=>`<img src="${escapeHtml(url)}" alt="Gallery preview" onerror="this.style.display='none'">`).join('');
}
async function uploadHomeGalleryImages(){
  const input=document.getElementById('homeGalleryUploader');
  const box=document.getElementById('home_gallery');
  const status=document.getElementById('homeGalleryUploadStatus');
  if(!input||!box||!input.files?.length)return alert('Please select gallery images first.');
  const urls=[]; status.textContent=`Uploading ${input.files.length} image(s)...`;
  for(const file of input.files){
    if(!file.type.startsWith('image/'))continue;
    const fd=new FormData();fd.append('file',file);fd.append('folder','home-gallery');
    try{const r=await fetch(`${API_BASE}/api/admin/upload-image`,{method:'POST',headers:uploadHeaders(),body:fd});const out=await r.json();if(!r.ok)throw new Error(out.error||'Upload failed');urls.push(out.url);status.textContent=`Uploaded ${urls.length} of ${input.files.length} image(s)...`;}catch(err){alert(err.message);}
  }
  if(urls.length){box.value=[box.value.trim(),...urls].filter(Boolean).join('\n');previewHomeGallery();status.textContent='Gallery upload completed. Save Global Settings to publish.';}else status.textContent='No images uploaded.';
  input.value='';
}
document.addEventListener('input',e=>{if(e.target?.id==='home_gallery')previewHomeGallery();});
