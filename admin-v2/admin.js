...
/* =====================================================
   CEYBREEZ ADMIN V11 ENTERPRISE SHELL PATCH - FIXED
   Stable V10 functions preserved. Adds clean router,
   no dashboard duplicate, and detailed dashboard summary.
===================================================== */
(function(){
  const V11_TABS = [
    "dashboardTab","destinationsTab","propertiesTab","servicesTab","inquiriesTab",
    "bookingsTab","reviewsTab","reportsTab","pageControlTab","settingsTab"
  ];
  ...
  const TAB_ALIASES = {
    ...
    pageControlTab: "pageControlTab",
    inquiriesTab: "inquiriesTab",
    propertyInquiries: "inquiriesTab",
    tourInquiries: "inquiriesTab",
    bookingsTab: "bookingsTab",
    propertyBookings: "bookingsTab",
    tourBookings: "bookingsTab",
    reviewsTab: "reviewsTab",
    reportsTab: "reportsTab",
    settingsTab: "settingsTab"
  };

  const originalShowTab = typeof window.showTab === "function" ? window.showTab : null;

  function v11DateKey(value){
    if(!value) return "";
  ...
  function v11ActiveBookings(bookings){
    return bookings.filter(x => String(x.status || "Booked").toLowerCase() === "booked");
  }

  function v11CheckIns(bookings, dateKey){
    return v11ActiveBookings(bookings).filter(x => v11DateKey(x.dateFrom) === dateKey);
  }

  function v11CheckOuts(bookings, dateKey){
    return v11ActiveBookings(bookings).filter(x => v11DateKey(x.dateTo) === dateKey);
  }

  function v11InHouse(bookings, dateKey){
    return v11ActiveBookings(bookings).filter(x => {
      const from = v11DateKey(x.dateFrom);
      const to = v11DateKey(x.dateTo);
      return from && to && from <= dateKey && dateKey < to;
    });
  }

  function v11BookingLine(item){
    const ref = item.reference || item.id || "-";
    const guest = item.guestName || "Guest";
    const place = item.itemName || item.serviceType || "-";
    return `<li><strong>${escapeHtml(guest)}</strong><span>${escapeHtml(place)}</span><small>${escapeHtml(ref)}</small></li>`;
  }

  function v11List(title, items, emptyText){
    return `
      <div class="v11-ops-card">
        <h4>${title}</h4>
        ${items.length ? `<ul>${items.slice(0,8).map(v11BookingLine).join("")}</ul>` : `<p class="v11-empty-mini">${emptyText}</p>`}
      </div>
    `;
  }

  window.v11RenderDashboard = function(){
    const cardBox = document.getElementById("v11DashboardCards");
    const movementBox = document.getElementById("v11MovementCards");
    const opsBox = document.getElementById("v11TodayOps");
    ...
    const destinations = Array.isArray(window.allDestinations) ? window.allDestinations : (typeof allDestinations !== "undefined" ? allDestinations : []);

    const active = v11ActiveBookings(bookings);
    const newInquiries = inquiries.filter(x => String(x.status || "New").toLowerCase() === "new").length;
    const today = v11OffsetDate(0);
    const tomorrow = v11OffsetDate(1);
    const yesterday = v11OffsetDate(-1);

    const todayIn = v11CheckIns(bookings, today);
    const todayOut = v11CheckOuts(bookings, today);
    const tomorrowIn = v11CheckIns(bookings, tomorrow);
    const tomorrowOut = v11CheckOuts(bookings, tomorrow);
    const yesterdayIn = v11CheckIns(bookings, yesterday);
    const yesterdayOut = v11CheckOuts(bookings, yesterday);
    const inHouseToday = v11InHouse(bookings, today);

    if(cardBox){
      cardBox.innerHTML = `
        <div class="dashboard-card card-new"><h3>New Inquiries</h3><div class="value">${newInquiries}</div><small>Need follow-up</small></div>
        <div class="dashboard-card card-booked"><h3>Active Bookings</h3><div class="value">${active.length}</div><small>Current booked records</small></div>
        <div class="dashboard-card"><h3>Today Check-in</h3><div class="value">${todayIn.length}</div><small>${v11HumanDate(today)}</small></div>
        <div class="dashboard-card"><h3>Today Check-out</h3><div class="value">${todayOut.length}</div><small>${v11HumanDate(today)}</small></div>
        <div class="dashboard-card"><h3>In-house Today</h3><div class="value">${inHouseToday.length}</div><small>Staying tonight</small></div>
        <div class="dashboard-card"><h3>Tomorrow Check-in</h3><div class="value">${tomorrowIn.length}</div><small>${v11HumanDate(tomorrow)}</small></div>
        <div class="dashboard-card"><h3>Properties</h3><div class="value">${properties.length}</div><small>CMS records</small></div>
        <div class="dashboard-card"><h3>Tour Locations</h3><div class="value">${destinations.length}</div><small>CMS records</small></div>
      `;
    }

    if(movementBox){
      movementBox.innerHTML = `
        <div class="v11-movement-card yesterday"><span>Yesterday</span><strong>${v11HumanDate(yesterday)}</strong><p>Check-in: <b>${yesterdayIn.length}</b></p><p>Check-out: <b>${yesterdayOut.length}</b></p></div>
        <div class="v11-movement-card today"><span>Today</span><strong>${v11HumanDate(today)}</strong><p>Check-in: <b>${todayIn.length}</b></p><p>Check-out: <b>${todayOut.length}</b></p><p>In-house: <b>${inHouseToday.length}</b></p></div>
        <div class="v11-movement-card tomorrow"><span>Tomorrow</span><strong>${v11HumanDate(tomorrow)}</strong><p>Check-in: <b>${tomorrowIn.length}</b></p><p>Check-out: <b>${tomorrowOut.length}</b></p></div>
      `;
    }

    if(opsBox){
      opsBox.innerHTML = `
        ${v11List("Today Check-ins", todayIn, "No check-ins today")}
        ${v11List("Today Check-outs", todayOut, "No check-outs today")}
        ${v11List("Tomorrow Check-ins", tomorrowIn, "No check-ins tomorrow")}
      `;
    }
  };

  function v11HideAllTabs(){
    V11_TABS.forEach(id => document.getElementById(id)?.classList.add("hidden"));
  }

  function v11SetActive(tab){
    document.querySelectorAll(".v11-sidebar button[data-tab]").forEach(btn => {
      const btnNormalized = TAB_ALIASES[btn.dataset.tab] || btn.dataset.tab;
      btn.classList.toggle("active", btnNormalized === tab);
    });
  }
  ...
  window.showTab = function(tab){
    ...
    if(normalized === "dashboardTab"){
      document.getElementById("dashboardTab")?.classList.remove("hidden");
      v11SetActive("dashboardTab");
      window.scrollTo({top:0, behavior:"instant"});
      window.v11RenderDashboard();
      return;
    }
    ...
    /* original V10 router does not know dashboardTab. Force clean visibility again. */
    document.getElementById("dashboardTab")?.classList.add("hidden");
    V11_TABS.forEach(id => {
      if(id !== normalized && !(normalized === "inquiriesTab" && id === "inquiriesTab") && !(normalized === "bookingsTab" && id === "bookingsTab")){
        document.getElementById(id)?.classList.add("hidden");
      }
    });
    document.getElementById(normalized)?.classList.remove("hidden");

    v11SetActive(normalized);
    window.scrollTo({top:0, behavior:"instant"});

    if(normalized === "destinationsTab") loadDestinations?.();
    if(normalized === "propertiesTab") loadProperties?.();
    if(normalized === "servicesTab") loadServices?.();
    if(normalized === "inquiriesTab") loadInquiries?.();
    if(normalized === "bookingsTab") loadBookings?.();
    if(normalized === "pageControlTab") { loadSiteContent?.(); loadPageSections?.(); }
    if(normalized === "reportsTab") renderReports?.();

    setTimeout(window.v11RenderDashboard, 80);
  };
  ...
  if(originalLoadAll){
    window.loadAll = async function(){
      ...
      setTimeout(window.v11RenderDashboard, 120);
      return result;
    };
  }
