(() => {
  ...
  async function extractBodyAndStyles(html,page,sourceUrl){
    ...
    if(page.id==='welcome'){
      ...
      if(welcome) return {html:welcome.outerHTML,css:await collectCss(doc,baseUrl),bodyAttrs:getBodyAttrs(doc)};
      return fallbackPage(page);
    }
    return {html:doc.body.innerHTML,css:await collectCss(doc,baseUrl),bodyAttrs:getBodyAttrs(doc)};
  }
