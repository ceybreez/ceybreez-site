(() => {
  ...
  function draftKey(id){ return `ceybreez-builder-pro-v1.4:${id}`; }
  ...
  async function extractBodyAndStyles(html,page,sourceUrl){
    ...
    if(page.id==='home'){
      ...
      if(homeRoot){
        ...
        // Keep every Home section, including elements placed outside #mainSite.
        // Dynamic sections are given visible editor placeholders because the live
        // page scripts are intentionally not executed inside GrapesJS.
        hydrateHomeEditorPlaceholders(homeRoot);
        const extras=[...doc.body.children]
          .filter(el=>el!==homeRoot && !el.matches('#welcomeScreen,.welcome-screen,[data-welcome],.welcome-overlay,script'))
          .map(el=>el.outerHTML).join('');
        const canvasFix=`
/* Builder canvas: show and scroll the complete Home page */
html,body{min-height:100%!important;height:auto!important;overflow-x:hidden!important;overflow-y:auto!important;}
body[data-builder-view="home"] #mainSite{display:block!important;min-height:100vh!important;height:auto!important;overflow:visible!important;opacity:1!important;visibility:visible!important;}
body[data-builder-view="home"] .cbp-runtime-placeholder{min-height:180px;display:grid;place-items:center;padding:30px;margin:18px;border:2px dashed rgba(11,107,93,.45);background:rgba(238,248,245,.92);text-align:center;border-radius:14px;}
`;
        return {html:homeRoot.outerHTML+extras,css:css+canvasFix,bodyAttrs:{...getBodyAttrs(doc),'data-page':'home','data-builder-view':'home'}};
      }
  ...
  function hydrateHomeEditorPlaceholders(homeRoot){
    const placeholders=[
      ['#featuredStaysGrid','Featured Stays','Live property cards load here from the CEYBREEZ API.'],
      ['#featuredDestinationsGrid','Popular Destinations','Live destination cards load here from the CEYBREEZ API.'],
      ['#featuredServicesGrid','Featured Services','Live service cards load here from the CEYBREEZ API.'],
      ['[data-review-grid]','Guest Reviews','Live featured guest reviews load here.'],
      ['[data-reels-grid]','Travel Reels','Live reels and guest moments load here.'],
      ['#experiences .grid','Our Experiences','Live experience cards load here.']
    ];
    placeholders.forEach(([selector,title,text])=>{
      const el=homeRoot.querySelector(selector);
      if(el && !el.children.length){
        el.innerHTML=`<div class="cbp-runtime-placeholder" data-ceybreez-runtime-placeholder="${title}"><div><strong>${title}</strong><p>${text}</p></div></div>`;
      }
    });
  }
