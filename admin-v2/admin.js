...
/* =====================================================
   CEYBREEZ ADMIN V11 ENTERPRISE SHELL PATCH
   Stable V10 functions preserved. This patch only adds
   dashboard routing, sidebar active state and summary cards.
===================================================== */
(function(){
  const V11_TABS = [
  ...
  window.v11RenderDashboard = function(){
    const box = document.getElementById("v11DashboardCards");
    if(!box) return;
    ...
    const booked = bookings.filter(x => String(x.status || "").toLowerCase() === "booked").length;
    const newInquiries = inquiries.filter(x => String(x.status || "New").toLowerCase() === "new").length;

    box.innerHTML = `
      <div class="dashboard-card card-new"><h3>New Inquiries</h3><div class="value">${newInquiries}</div></div>
      <div class="dashboard-card card-booked"><h3>Active Bookings</h3><div class="value">${booked}</div></div>
      <div class="dashboard-card"><h3>Properties</h3><div class="value">${properties.length}</div></div>
      <div class="dashboard-card"><h3>Tour Locations</h3><div class="value">${destinations.length}</div></div>
      <div class="dashboard-card"><h3>Total Inquiries</h3><div class="value">${inquiries.length}</div></div>
      <div class="dashboard-card"><h3>Total Bookings</h3><div class="value">${bookings.length}</div></div>
    `;
  };
  ...
  function v11SetActive(tab){
    document.querySelectorAll(".v11-sidebar button[data-tab]").forEach(btn => {
      ...
      const active = btnTab === tab ||
        (btnTab === "properties" && tab === "propertiesTab") ||
        (btnTab === "destinations" && tab === "destinationsTab") ||
        (btnTab === "services" && tab === "servicesTab") ||
        (btnTab === "pageControl" && tab === "pageControlTab");
...
/* =====================================================
   V11 COMPLETE FINALIZER
===================================================== */
(function(){
  function v11BodyState(){
    const panel = document.getElementById('adminPanel');
    document.body.classList.toggle('v11-admin-open', !!panel && !panel.classList.contains('hidden'));
  }

  const oldLoginAdmin = typeof window.loginAdmin === 'function' ? window.loginAdmin : null;
  if(oldLoginAdmin){
    window.loginAdmin = function(){
      const r = oldLoginAdmin.apply(this, arguments);
      setTimeout(v11BodyState, 50);
      setTimeout(() => window.showTab && window.showTab('dashboardTab'), 300);
      return r;
    };
  }

  const oldLogoutAdmin = typeof window.logoutAdmin === 'function' ? window.logoutAdmin : null;
  if(oldLogoutAdmin){
    window.logoutAdmin = function(){
      document.body.classList.remove('v11-admin-open');
      return oldLogoutAdmin.apply(this, arguments);
    };
  }

  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(v11BodyState, 100);
    setTimeout(v11BodyState, 500);
    setTimeout(() => {
      const panel = document.getElementById('adminPanel');
      if(panel && !panel.classList.contains('hidden') && window.showTab){
        window.showTab('dashboardTab');
      }
    }, 700);
  });
