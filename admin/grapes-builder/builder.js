...
const API_BASE='https://ceybreez-contact-api.ceybreez.workers.dev', PREFIX='CEYBREEZ_LIVE_BUILDER_V12:';
...
d.addEventListener('click',onFrameClick,true);d.addEventListener('dblclick',onFrameDblClick,true);d.addEventListener('mouseover',e=>{if(e.target!==d.body&&e.target!==d.documentElement&&!e.target.closest('#cb-selection-overlay'))e.target.dataset.cbHover='1'},true);d.addEventListener('mouseout',e=>delete e.target.dataset.cbHover,true);d.addEventListener('scroll',positionSelectionOverlay,true);frame.contentWindow.addEventListener('resize',positionSelectionOverlay);applyDraft();buildSections();buildLayers();
...
function owningSection(el){return el?.closest?.('[data-section]')||null}
function relativePath(el,root){
 if(!el||!root||el===root)return ':scope';
 const parts=[];let n=el;
 while(n&&n!==root){let p=n.tagName.toLowerCase();if(n.id){p='#'+CSS.escape(n.id);parts.unshift(p);break}const classes=[...n.classList].filter(x=>!x.startsWith('cb-')&&x!=='entered').slice(0,2);if(classes.length)p+='.'+classes.map(CSS.escape).join('.');const same=n.parentElement?[...n.parentElement.children].filter(x=>x.tagName===n.tagName):[];if(same.length>1)p+=`:nth-of-type(${same.indexOf(n)+1})`;parts.unshift(p);n=n.parentElement}
 return parts.join(' > ')
}
function ensureDraftEntry(el){
 ...
 const sectionKey=section?.dataset.section||'__visual_global';
 const path=previous.path||structuralPath(el);
 const relSelector=section?relativePath(el,section):structuralPath(el);
 draft[key]={editId,path,sectionKey,relSelector,selector:`[data-cb-edit-id="${editId}"]`,html:previous.html,attrs:previous.attrs||{},style:previous.style||'',deviceStyles:previous.deviceStyles||{tablet:{},mobile:{}}};
...
function authHeaders(){
 ...
 if(!token)throw new Error('Admin token not found. Please log in to Admin again.');
 return {'Content-Type':'application/json',Authorization:`Bearer ${token}`};
}
function apiPage(){return pageKey==='welcome'?'home':pageKey==='home'?'home':String(pageKey).replace(/\.html$/,'')}
function cleanPublishRecord(p){
 const attrs={};Object.entries(p.attrs||{}).forEach(([k,v])=>{if(!k.startsWith('data-cb-'))attrs[k]=v});
 return {selector:p.relSelector||p.path||p.selector,html:p.html,attrs,style:p.style||'',deviceStyles:p.deviceStyles||{tablet:{},mobile:{}}};
}
async function publishLive(){
 try{
  if(selected)recordChange(selected);
  const draft=getDraft(), records=Object.values(draft);
  if(!records.length)throw new Error('There are no saved edits to publish.');
  const page=apiPage(), headers=authHeaders();
  setStatus(`Publishing ${records.length} edit(s) to the live website…`);
  const res=await fetch(`${API_BASE}/api/admin/page-sections?page=${encodeURIComponent(page)}`,{headers});
  const data=await res.json();if(!res.ok)throw new Error(data.error||'Could not load page sections');
  const items=Array.isArray(data)?data:[];
  const grouped={};records.forEach(p=>{const key=p.sectionKey||'__visual_global';(grouped[key]||(grouped[key]=[])).push(cleanPublishRecord(p))});
  for(const [sectionKey,visualBuilderRecords] of Object.entries(grouped)){
   const existing=items.find(x=>x.sectionKey===sectionKey);
   let settings={};try{settings=typeof existing?.settings==='string'?JSON.parse(existing.settings||'{}'):(existing?.settings||{})}catch{settings={}}
   settings.visualBuilderRecords=visualBuilderRecords;
   const body=existing?{...existing,settings}:{page,sectionKey,sectionType:'visual-builder',title:sectionKey==='__visual_global'?'Visual Builder Global':'',subtitle:'',content:'',buttonText:'',buttonUrl:'',mediaUrl:'',backgroundType:'color',backgroundColor:'',backgroundImage:'',textColor:'',headingColor:'',buttonColor:'',fontFamily:'',fontSize:'',sortOrder:9999,active:true,settings};
   const wr=await fetch(`${API_BASE}/api/admin/page-sections`,{method:'POST',headers,body:JSON.stringify(body)});const out=await wr.json().catch(()=>({}));if(!wr.ok)throw new Error(out.error||`Publish failed for ${sectionKey}`);
  }
  setStatus(`Published successfully. ${records.length} edit(s) are now saved to the live Page Sections API.`);
 }catch(err){setStatus('Publish failed: '+err.message,true)}
}
$('publishBtn').onclick=publishLive;
