...
(() => {
  ...
  const state = {
    ...
    elementStyles: {}, customElements: [], previewController: null, initialized: false, resizeTimer: null
  };
  ...
  function installPreviewEditor() {
    ...
    try{
      ...
      ro.observe(doc.documentElement); ro.observe(doc.body);
      state.previewController.signal.addEventListener('abort',()=>ro.disconnect(),{once:true});
    }catch(_){/* ResizeObserver is optional */}
  ...
  function fitPreview(){
    ...
    const viewport=DEVICE_WIDTHS[state.selectedDevice]||1440;
    const available=Math.max(280,panel.clientWidth-36);
    const scale=Math.min(1,available/viewport);
    frame.style.width=`${viewport}px`;
    frame.style.transform=`scale(${scale})`;
    frame.style.transformOrigin='top left';
    wrap.style.width=`${Math.round(viewport*scale)}px`;
    const doc=frame.contentDocument;
    let contentHeight=800;
    if(doc){
      const body=doc.body, html=doc.documentElement;
      contentHeight=Math.max(body?.scrollHeight||0,body?.offsetHeight||0,html?.scrollHeight||0,html?.offsetHeight||0,800);
    }
    frame.style.height=`${contentHeight}px`;
    wrap.style.height=`${Math.ceil(contentHeight*scale)}px`;
    const badge=$('pbxFitBadge');
    if(badge)badge.textContent=`${Math.round(scale*100)}% Auto Fit`;
  }
  function scheduleFit(){clearTimeout(state.resizeTimer);state.resizeTimer=setTimeout(fitPreview,60);}
  function ensureFitBadge(){
    if($('pbxFitBadge'))return;
    const panel=document.querySelector('.pb2-preview-panel');
    if(!panel)return;
    const badge=document.createElement('div');badge.id='pbxFitBadge';badge.className='pbx-fit-badge';badge.textContent='Auto Fit';panel.prepend(badge);
  }
  function setDevice(device) {
    ...
    applyAllToPreview(); renderInspector(); scheduleFit();
  }
