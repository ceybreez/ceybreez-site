...
function recordChange(el){if(!el)return;const s=selectorFor(el),draft=getDraft();draft[s]={selector:s,html:el.innerHTML,attrs:{},style:el.getAttribute('style')||''};['href','src','class','id','data-cb-user-edited'].forEach(a=>{if(el.hasAttribute(a))draft[s].attrs[a]=el.getAttribute(a)});localStorage.setItem(PREFIX+pageKey,JSON.stringify(draft));updateInspector();buildLayers()}
...
$('hideBtn').onclick=()=>mutate(el=>setUserStyle(el,'display',el.style.display==='none'?'':'none'));$('resetElementBtn').onclick=()=>{if(!selected)return;const draft=getDraft();delete draft[selectorFor(selected)];localStorage.setItem(PREFIX+pageKey,JSON.stringify(draft));loadPage()};
