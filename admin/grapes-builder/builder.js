...
body.cb-builder-mobile main>section{padding-left:16px!important;padding-right:16px!important;width:100%!important;max-width:100%!important}
body.cb-builder-mobile section,body.cb-builder-mobile section>*{max-width:100%!important;min-width:0!important}
body.cb-builder-mobile h1{font-size:clamp(1.9rem,10vw,3rem)!important;line-height:1.08!important}
body.cb-builder-mobile h2{font-size:clamp(1.55rem,8vw,2.35rem)!important;line-height:1.15!important}
body.cb-builder-mobile h3{font-size:clamp(1.15rem,5vw,1.6rem)!important}
...
function doMove(e){if(!resizeState||resizeState.mode!=='move'||!selected)return;e.preventDefault();const x=resizeState.tx+(e.clientX-resizeState.startX),y=resizeState.ty+(e.clientY-resizeState.startY);selected.style.transform=`translate(${Math.round(x)}px, ${Math.round(y)}px)`;positionSelectionOverlay()}
...
function selectElement(el){if(!el||el.closest?.('#cb-selection-overlay'))return;if(selected)delete selected.dataset.cbSelected;selected=el;selected.dataset.cbSelected='1';$('selectedLabel').textContent=selectorFor(el);updateInspector();updateContextPanels();positionSelectionOverlay()}
...
$('justifyContent').onchange=e=>mutate(el=>el.style.justifyContent=e.target.value);$('alignItems').onchange=e=>mutate(el=>el.style.alignItems=e.target.value);document.querySelectorAll('[data-align]').forEach(b=>b.onclick=()=>{mutate(el=>el.style.textAlign=b.dataset.align);$('textAlign').value=b.dataset.align;document.querySelectorAll('[data-align]').forEach(x=>x.classList.toggle('active',x===b));});
...
$('useTextColor').addEventListener('change',e=>{ $('colorValue').disabled=!e.target.checked;mutate(el=>{if(e.target.checked)el.style.color=$('colorValue').value;else el.style.removeProperty('color')});});
$('useBgColor').addEventListener('change',e=>{ $('bgValue').disabled=!e.target.checked;mutate(el=>{if(e.target.checked)el.style.backgroundColor=$('bgValue').value;else el.style.removeProperty('background-color')});});
