...
const API_BASE='https://ceybreez-contact-api.ceybreez.workers.dev', PREFIX='CEYBREEZ_LIVE_BUILDER_V9:';
...
function ensureDraftEntry(el){const s=selectorFor(el),draft=getDraft();const old=draft[s]||{};draft[s]={selector:s,html:old.html,attrs:old.attrs||{},style:old.style||'',deviceStyles:old.deviceStyles||{tablet:{},mobile:{}}};return {s,draft,entry:draft[s]}}
function recordChange(el){if(!el)return;const {draft,entry}=ensureDraftEntry(el);entry.html=el.innerHTML;entry.attrs={};['href','src','class','id','data-cb-user-edited'].forEach(a=>{if(el.hasAttribute(a))entry.attrs[a]=el.getAttribute(a)});if(currentDevice==='desktop')entry.style=el.getAttribute('style')||'';saveDraftObject(draft);applyDeviceOverrides();updateInspector();buildLayers()}
...
function applyDraft(){const d=frame.contentDocument,draft=getDraft();Object.values(draft).forEach(p=>{try{const el=d.querySelector(p.selector);if(!el)return;if(p.html!==undefined)el.innerHTML=p.html;Object.entries(p.attrs||{}).forEach(([k,v])=>el.setAttribute(k,v));if(p.style!==undefined)el.setAttribute('style',p.style)}catch{}});applyDeviceOverrides()}
