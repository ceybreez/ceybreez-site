(function(){
  ...
  async function renderAvailabilityManager(){
    ...
    if(!box) return;
    box.innerHTML=`<div class="v14-section-head"><div><h2>Availability Manager</h2><p>Loading property calendar and blocks...</p></div></div>`;
    try{
      ...
      const matrixRows=props.map(p=>{
        const cells=dates.map(d=>{
          ...
          if(b){ bookedCount++; return `<td class="v18-cell booked" title="${escapeHtml(b.reference||'Booking')} - ${escapeHtml(b.guestName||'')}">B</td>`; }
          ...
          if(bl){
            ...
            return `<td class="v18-cell ${cls}" title="${escapeHtml(bl.type||'Blocked')} - ${escapeHtml(bl.reason||'')}">${sh}</td>`;
          }
          freeCount++; return `<td class="v18-cell free">A</td>`;
        }).join("");
        return `<tr><td class="v18-property"><strong>${escapeHtml(p.name||'-')}</strong><small>${escapeHtml(p.type||'')}</small></td>${cells}</tr>`;
      }).join("");
      const propOptions=props.map(p=>`<option value="${escapeHtml(p.name||'')}">${escapeHtml(p.name||'')} (${escapeHtml(p.type||'')})</option>`).join("");
      const blockRows=blocks.length ? blocks.map(bl=>`<tr><td>${escapeHtml(bl.propertyName)}</td><td><span class="v18-badge ${availabilityTypeClass(bl.type)}">${escapeHtml(bl.type)}</span></td><td>${escapeHtml(bl.dateFrom)} → ${escapeHtml(bl.dateTo)}</td><td>${escapeHtml(bl.reason||'-')}</td><td><button class="delete-btn mini-btn" onclick="v18DeleteBlock('${escapeHtml(bl.id)}')">Delete</button></td></tr>`).join("") : `<tr><td colspan="5" class="empty-row">No owner / maintenance / private blocks saved yet.</td></tr>`;
      box.innerHTML=`
        <div class="v14-section-head v18-sticky-head">
          <div><h2>Availability Manager</h2><p>Bookings from live V10 + owner/maintenance/private blocks in Admin V18.</p></div>
        ...
        <section class="admin-section v18-block-panel">
          <div class="section-head"><div><h2>Block Dates</h2><p>Use for Owner Stay, Maintenance, Private or Hold. These blocks show inside Admin V18 availability.</p></div></div>
          <form id="v18BlockForm" class="v18-block-form">
            <select id="v18BlockProperty" required><option value="">Select Property</option>${propOptions}</select>
            <select id="v18BlockType" required><option value="Owner Stay">Owner Stay</option><option value="Maintenance">Maintenance</option><option value="Private">Private</option><option value="Hold">Hold</option></select>
            <input id="v18BlockFrom" type="date" required>
            <input id="v18BlockTo" type="date" required>
            <input id="v18BlockReason" placeholder="Reason / note">
            <button type="submit">Save Block</button>
          </form>
        </section>
        <section class="admin-section v18-matrix-panel">
          <div class="section-head"><div><h2>Availability Matrix</h2><p>A = Available, B = Booked, O = Owner, M = Maintenance, P = Private, H = Hold.</p></div><div class="v18-legend"><span class="free">A</span><span class="booked">B</span><span class="owner">O</span><span class="maintenance">M</span><span class="private">P</span><span class="hold">H</span></div></div>
          <div class="v18-matrix-scroll"><table class="v18-matrix"><thead><tr><th class="v18-property-head">Property</th>${dates.map(d=>`<th>${fmtDate(d).replace(' 2026','')}</th>`).join("")}</tr></thead><tbody>${matrixRows || `<tr><td colspan="${dates.length+1}" class="empty-row">No active properties found.</td></tr>`}</tbody></table></div>
        </section>
        <section class="admin-section">
          <div class="section-head"><div><h2>Saved Blocks</h2><p>Edit/delete owner and maintenance blocks.</p></div></div>
      ...
      box.innerHTML=`<div class="v14-section-head"><div><h2>Availability Manager</h2><p>${escapeHtml(err.message)}</p></div><button class="v14-refresh" onclick="v18RenderAvailability()">Retry</button></div>`;
    }
  }
  ...
  function ensureShell(){
    ...
    sidebar.innerHTML=`
      <div class="v14-brand"><div class="v14-logo">CB</div><div><strong>CeyBreez</strong><small>V18 · Availability</small></div></div>
      <nav class="v14-nav">
    ...
    // V18 availability is a real independent module section.
