(() => {
  ...
  function draftKey(id){ return `ceybreez-builder-pro:${id}`; }
  ...
  async function fetchOriginal(page){
    ...
    for(const url of candidates){
      try{ const r=await fetch(url,{cache:'no-store'}); if(r.ok){ const html=await r.text(); return extractBodyAndStyles(html,page,r.url || new URL(url, location.href).href); } }catch(_){ }
    }
    return fallbackPage(page);
  }

  function extractBodyAndStyles(html,page,sourceUrl){
    const doc=new DOMParser().parseFromString(html,'text/html');
    const baseUrl = sourceUrl || location.href;
    rewriteAssetUrls(doc, baseUrl);
    doc.querySelectorAll('script').forEach(s=>s.remove());
    if(page.id==='welcome'){
      const welcome=doc.querySelector('.welcome-screen,#welcomeScreen,[data-welcome],.welcome-overlay');
      if(welcome) return {html:welcome.outerHTML,css:collectCss(doc,baseUrl)};
      return fallbackPage(page);
    }
    return {html:doc.body.innerHTML,css:collectCss(doc,baseUrl)};
  }
  ...
  function collectCss(doc,baseUrl){
    const inline=[...doc.querySelectorAll('style')].map(x=>x.textContent).join('\n');
    const links=[...doc.querySelectorAll('link[rel="stylesheet"]')].map(x=>{
      const href=x.getAttribute('href');
      try{return `@import url("${new URL(href,baseUrl).href}");`;}catch(_){return '';}
    }).join('\n');
    return `${links}\n${inline}`;
  }
  ...
  async function loadPage(page,forceOriginal=false){
    ...
    try{
      ...
      else { const original=await fetchOriginal(page); editor.setComponents(original.html); editor.setStyle(original.css||''); }
